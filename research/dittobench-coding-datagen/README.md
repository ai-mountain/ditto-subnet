...
# Grade a local practice workspace. This is a convenience runner, not a secure
# production sandbox.
...
pack
allows edits only to `app.py` and exposes the fixed command IDs `visible-unit`
and `python-compile`. A harness never receives the temporary workspace path.
